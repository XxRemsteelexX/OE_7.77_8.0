...
// v9.3 Remora: segment directories for sequential low-memory search
vector<string> g_remora_segment_dirs;
bool g_remora_mode = false;

// v9.1: Get the correct storage.bin path for a global doc index
// In multi-segment mode, each segment has its own storage.bin
inline const string& get_storage_path_for_doc(uint32_t global_doc_idx) {
    if (g_multi_seg.loaded && !g_segment_storage_paths.empty()) {
        int seg_idx = find_segment_for_doc(g_multi_seg, global_doc_idx);
        if (seg_idx >= 0 && seg_idx < (int)g_segment_storage_paths.size()) {
            return g_segment_storage_paths[seg_idx];
        }
    }
    return global_storage_path;
}

// v9.1: Decompress chunk with automatic segment storage resolution
inline string decompress_chunk_auto(uint32_t global_doc_idx, uint64_t offset, uint64_t length) {
    return decompress_chunk(get_storage_path_for_doc(global_doc_idx), offset, length);
}
...
vector<Hit> scanner_filter_chunks(const string& question, const vector<Hit>& candidates) {
...

    {
        ...
        for (int i = 0; i < max_scan; i++) {
            if (candidates[i].doc_idx < global_corpus.docs.size()) {
                ...
                cands.push_back({i, "", doc.offset, doc.length, candidates[i].doc_idx});
            }
        }
    }
    ...
    for (auto& c : cands) {
        string text = decompress_chunk_auto(c.doc_idx, c.offset, c.length);
    ...
    for (const auto& c : cands) {
        prompts.push_back(
            "Question: " + question + "\n\n"
            "Chunk:\n" + c.text + "\n\n"
            "Does this chunk contain information relevant to answering the question? Reply YES or NO only. /no_think");
    }
    ...
    for (int batch_start = 0; batch_start < (int)prompts.size() && (int)filtered.size() < max_keep; batch_start += batch_size) {
        ...
        for (int j = 0; j < (int)batch_results.size(); j++) {
            ...
            // log first few scanner responses for debugging
            if (batch_start == 0 && j < 3) {
                cerr << "[scanner] cand " << cand_idx << " response: \""
                     << response.substr(0, 50) << "\"" << endl;
            }
...

    {
        ...
        for (int i = 0; i < max_preview && i < (int)hits.size(); i++) {
            if (hits[i].doc_idx < global_corpus.docs.size()) {
                ...
                string text = decompress_chunk_auto(hits[i].doc_idx, doc.offset, doc.length);
...
RefinementResult search_with_refinement(const string& question, int topk, int max_iter = 3) {
    ...
    for (int iter = 0; iter <= max_iter; iter++) {
        ...
        {
            shared_lock<shared_mutex> lock(corpus_mutex);
            // v9.3 Remora: sequential low-memory segment search
            if (g_remora_mode && !g_remora_segment_dirs.empty()) {
                auto remora = search_sequential_segments(
                    g_remora_segment_dirs, current_query, topk,
                    g_config.segments.early_stop_score);
                hits = std::move(remora.hits);
            // v9.1: prefer multi-segment search if loaded
            } else if (g_multi_seg.loaded) {
            ...
            if (!g_multi_seg.loaded && !g_remora_mode) {
...
json handle_chat(const string& question) {
...

    {
        ...
        for (const auto& hit : hits) {
            if (hit.doc_idx < global_corpus.docs.size()) {
                ...
                hit_metas.push_back({doc.id, doc.offset, doc.length, hit.score, hit.doc_idx});
            }
        }
    }
    ...
    for (const auto& meta : hit_metas) {
        string chunk_text = decompress_chunk_auto(meta.doc_idx, meta.offset, meta.length);
...
void run_http_server() {
    ...
    svr.Get("/health", [](const httplib::Request&, httplib::Response& res) {
        json health = {{"status", "ok"}, {"version", "9.1"}, {"creature_tier", get_creature_name()}, {"tentacles", get_tentacle_count()}};
        ...
        try {
            ...
            for (size_t i = 0; i < global_corpus.docs.size(); i++) {
                if (global_corpus.docs[i].id == chunk_id) {
                    const auto& doc = global_corpus.docs[i];
                    // v9.1: use correct storage.bin for multi-segment
                    const string& storage = get_storage_path_for_doc((uint32_t)i);
                    string text = decompress_chunk(storage, doc.offset, doc.length);
                    json result = {
                        {"chunk_id", chunk_id},
        ...
        try {
            ...
            if (use_refinement && !g_config.llm.api_key.empty()) {
                    ...
                    ? g_config.scanner.max_candidates : topk;
                {
                    ...
                    if (g_remora_mode && !g_remora_segment_dirs.empty()) {
                        auto remora = search_sequential_segments(
                            g_remora_segment_dirs, query, search_topk,
                            g_config.segments.early_stop_score);
                        hits = std::move(remora.hits);
                    } else if (g_multi_seg.loaded) {
        ...
        try {
            ...
            vector<Hit> hits;
            {
                ...
                if (is_self_referential_query(question)) {
                    ...
                    if (g_remora_mode && !g_remora_segment_dirs.empty()) {
                        auto remora = search_sequential_segments(
                            g_remora_segment_dirs, question, stream_search_topk,
                            g_config.segments.early_stop_score);
                        hits = std::move(remora.hits);
                    } else if (g_multi_seg.loaded) {
            ...
            struct StreamHitMeta { string id; uint64_t offset; uint64_t length; double score; uint32_t doc_idx; };
...
int main(int argc, char** argv) {
    ...
    cout << "OceanEterna Chat Server v9.1 (Cap-100 + Hapax Filter + TF BM25)" << endl;
    cout << "============================================================\n" << endl;
    ...
    // If corpus_dir/meta.json exists with segment definitions, load as multi-segment
    {
        ...
        if (fs::exists(meta_path)) {
            try {
                ...
                json meta = json::parse(mf);
                if (meta.contains("segments") && meta["segments"].is_array()) {
                    // v9.3 Remora: sequential mode — just store dirs, don't load
                    if (g_config.segments.mode == "sequential") {
                        cout << "\n[remora] Registering " << meta["segments"].size()
                             << " segments (lazy-load mode)" << endl;
                        for (const auto& seg_def : meta["segments"]) {
                            string seg_dir = seg_def.value("dir", "");
                            if (!seg_dir.empty() && seg_dir[0] != '/') {
                                seg_dir = corpus_dir + "/" + seg_dir;
                            ...
                            if (fs::exists(seg_dir + "/stems.idx")) {
                                g_remora_segment_dirs.push_back(seg_dir);
                                g_segment_storage_paths.push_back(seg_dir + "/storage.bin");
                                cout << "[remora] Segment " << g_remora_segment_dirs.size() - 1
                                     << ": " << seg_dir << endl;
                            }
                        }
                        g_remora_mode = true;
                        g_multi_seg.loaded = true;  // signal multi-segment active for dispatch
                        cout << "[remora] " << g_remora_segment_dirs.size()
                             << " segments registered, peak RAM = ~1 segment" << endl;
                    } else {
                    // parallel mode (Kraken/Leviathan)
                    ...
                    for (const auto& seg_def : meta["segments"]) {
                        ...
                        // load keyword dict + docs from segment's manifest.bin
                        // docs are appended to global_corpus so /search can resolve chunk_ids
                        ...
                        if (fs::exists(seg_manifest_bin)) {
                            ...
                            // append segment docs to global_corpus
                            // (skip segment 0 — its docs are already in global_corpus from primary load)
                            if (seg_id != 0 || cum_offset > 0) {
                                size_t base = global_corpus.docs.size();
                                for (size_t di = 0; di < bc.docs.size(); di++) {
                                    auto& doc = bc.docs[di];
                                    // store which segment this doc belongs to (for storage.bin path)
                                    uint32_t global_idx = base + di;
                                    chunk_id_to_index[doc.id] = global_idx;
                                    global_corpus.docs.push_back(move(doc));
                                }
                                cout << "[multi-segment] Appended " << bc.docs.size()
                                     << " docs from segment " << seg_id
                                     << " (global_corpus now " << global_corpus.docs.size() << ")" << endl;
                            }
                        }
