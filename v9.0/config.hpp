...
struct Config {
    ...
    struct {
        ...
        int max_candidates = 64;   // how many BM25 results to scan
        int max_keep = 8;          // max YES results to keep
        int max_tokens = 5;        // scanner only needs YES/NO
    ...
    struct {
        ...
        double early_stop_score = 0.0;   // remora: stop if best BM25 score >= this (0 = search all)
    } segments;
};
    ...
    if (f.is_open()) {
        try {
            ...
            if (j.contains("scanner")) {
                cfg.scanner.enabled = j["scanner"].value("enabled", cfg.scanner.enabled);
                cfg.scanner.model = j["scanner"].value("model", cfg.scanner.model);
                cfg.scanner.url = j["scanner"].value("url", cfg.scanner.url);
                cfg.scanner.max_candidates = j["scanner"].value("max_candidates", cfg.scanner.max_candidates);
                cfg.scanner.max_keep = j["scanner"].value("max_keep", cfg.scanner.max_keep);
                cfg.scanner.max_tokens = j["scanner"].value("max_tokens", cfg.scanner.max_tokens);
            ...
            if (j.contains("segments")) {
                ...
                cfg.segments.early_stop_score = j["segments"].value("early_stop_score", cfg.segments.early_stop_score);
            }
