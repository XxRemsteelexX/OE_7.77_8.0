...
inline std::pair<std::string, double> query_scanner_llm(const std::string& prompt) {
    ...
    json request;
    try {
        request["model"] = g_config.scanner.model;
        request["messages"] = {{{"role", "user"}, {"content", prompt}}};
        request["temperature"] = g_config.scanner.temperature;
        request["max_tokens"] = g_config.scanner.max_tokens;
        request["stream"] = false;
    } catch (...) {
        curl_easy_cleanup(curl);
        return {"ERROR", 0};
    ...
    curl_easy_setopt(curl, CURLOPT_URL, g_config.scanner.url.c_str());

    struct curl_slist *headers = NULL;
    headers = curl_slist_append(headers, "Content-Type: application/json");

    // scanner uses same API key as main LLM (OCEAN_API_KEY)
    if (!g_config.llm.api_key.empty()) {
        std::string auth_header = "Authorization: Bearer " + g_config.llm.api_key;
        headers = curl_slist_append(headers, auth_header.c_str());
    }
    ...
    curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, curl_write_cb);
    curl_easy_setopt(curl, CURLOPT_WRITEDATA, &response_string);
    curl_easy_setopt(curl, CURLOPT_TIMEOUT, (long)g_config.scanner.timeout_sec);
    ...
    try {
        json response = json::parse(response_string);
        if (!response.contains("choices") || response["choices"].empty()) {
            return {"ERROR: No choices in scanner response", elapsed_ms};
        }
        auto& choice = response["choices"][0];
        if (!choice.contains("message") || !choice["message"].contains("content")) {
            return {"ERROR: No content in scanner response", elapsed_ms};
        }
        if (choice["message"]["content"].is_null()) {
            return {"ERROR: Scanner returned null", elapsed_ms};
        }
        std::string answer = choice["message"]["content"].get<std::string>();
        // trim
        answer.erase(0, answer.find_first_not_of(" \n\r\t"));
        if (!answer.empty()) answer.erase(answer.find_last_not_of(" \n\r\t") + 1);
        return {answer, elapsed_ms};
    } catch (const std::exception& e) {
        return {"ERROR: " + std::string(e.what()), elapsed_ms};
    }
}

// Batch scanner LLM calls using curl_multi for parallelism
// Returns vector of {response, elapsed_ms} in same order as prompts
inline std::vector<std::pair<std::string, double>> query_scanner_llm_batch(
    const std::vector<std::string>& prompts) {

    int n = (int)prompts.size();
    std::vector<std::pair<std::string, double>> results(n, {"ERROR", 0});
    ...
    std::vector<HandleData> handles(n);

    for (int i = 0; i < n; i++) {
        handles[i].curl = curl_easy_init();
        if (!handles[i].curl) continue;

        json request;
        request["model"] = g_config.scanner.model;
        request["messages"] = {{{"role", "user"}, {"content", prompts[i]}}};
        request["temperature"] = g_config.scanner.temperature;
        request["max_tokens"] = g_config.scanner.max_tokens;
        request["stream"] = false;

        handles[i].request_body = request.dump();
        handles[i].headers = NULL;
        handles[i].headers = curl_slist_append(handles[i].headers, "Content-Type: application/json");
        if (!g_config.llm.api_key.empty()) {
            std::string auth = "Authorization: Bearer " + g_config.llm.api_key;
            handles[i].headers = curl_slist_append(handles[i].headers, auth.c_str());
        }

        curl_easy_setopt(handles[i].curl, CURLOPT_URL, g_config.scanner.url.c_str());
        curl_easy_setopt(handles[i].curl, CURLOPT_HTTPHEADER, handles[i].headers);
        curl_easy_setopt(handles[i].curl, CURLOPT_POSTFIELDS, handles[i].request_body.c_str());
        curl_easy_setopt(handles[i].curl, CURLOPT_WRITEFUNCTION, curl_write_cb);
        curl_easy_setopt(handles[i].curl, CURLOPT_WRITEDATA, &handles[i].response_string);
        curl_easy_setopt(handles[i].curl, CURLOPT_TIMEOUT, (long)g_config.scanner.timeout_sec);

        curl_multi_add_handle(multi, handles[i].curl);
    }
    ...
    for (int i = 0; i < n; i++) {
        if (!handles[i].curl) continue;
        try {
            json resp = json::parse(handles[i].response_string);
            if (resp.contains("choices") && !resp["choices"].empty()) {
                ...
                if (choice.contains("message") && choice["message"].contains("content") &&
                    ...
                    answer.erase(0, answer.find_first_not_of(" \n\r\t"));
                    if (!answer.empty()) answer.erase(answer.find_last_not_of(" \n\r\t") + 1);
                    results[i] = {answer, batch_ms};
                }
            }
        } catch (...) {
            results[i] = {"ERROR: parse failed", batch_ms};
        }
        curl_multi_remove_handle(multi, handles[i].curl);
        curl_slist_free_all(handles[i].headers);
